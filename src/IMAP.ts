...
export class Worker {
  ...
  // For GMAIL 
  public async listMessagesOfGmail(mailboxName: string): Promise<IMessage[]> {
    ...
    // We have to select the mailbox first.  This gives us the message count.
    const mailbox: any = await client.selectMailbox(`[Gmail]/${mailboxName}`);
    console.log(`IMAP.Worker.listMessages(): Message count = ${mailbox.exists}`);
  ...
  public async getMessageBody(inCallOptions: ICallOptions): Promise<any> {
    ...
    const {html, subject, date}: ParsedMail = await simpleParser(messages[0]["body[]"]);
  ...
  public async getSelectedMailbox(): Promise<any> {
    
    console.log("IMAP.Worker.getSelectedMailbox()");

    const client: any = await this.connectToServer();

    const draftMailbox: any = await client.selectMailbox('[Gmail]/Starred')

    const messages: any[] = await client.listMessages(
      '[Gmail]/Starred',
      `${draftMailbox.exists - 5}:*`,
      ["uid", "envelope"]
    );
  ...
  public async getSelectedMailboxMessageBody(id: number): Promise<any> {
